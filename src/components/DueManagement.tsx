...
import { Customer, SalesRep, DuePaymentLog, UITheme } from '../types';
...
export const DueManagement: React.FC<DueManagementProps> = ({
  ...
  const dueCustomers = customers
    .filter((c) => {
      ...
      const matchesSeller = selectedSellerFilter === 'সব' || c.assignedSellerId === selectedSellerFilter;
  ...
  // Calculate Due Grouped by Seller (only active due customers)
  const sellerWiseDue = sellers.map((seller) => {
    const sellerCusts = customers.filter((c) => c.assignedSellerId === seller.id && c.currentDue > 0);
            ...
            <table className="min-w-[620px] w-full text-left text-xs whitespace-nowrap">
              <thead>
                <tr className="border-b border-slate-700 text-slate-400 font-medium pb-2">
                  ...
                  <th className="pb-3 px-3">সেলার</th>
