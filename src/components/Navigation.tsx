...
export type NavTab = 'dashboard' | 'pos' | 'stock' | 'due' | 'sales' | 'pending' | 'reports' | 'users' | 'features' | 'sms' | 'catalog' | 'seller-tracking' | 'shops';
...
export const Navigation: React.FC<NavigationProps> = ({
  ...
  return (
    <nav className="bg-slate-900 border-b border-slate-800 sticky top-0 z-30 shadow-lg md:hidden">
      <div className="max-w-7xl mx-auto px-3 sm:px-6">
        <div className="flex items-center justify-start py-2">
          <div className={`grid ${visibleTabs.length === 4 ? 'grid-cols-4' : visibleTabs.length === 3 ? 'grid-cols-3' : 'grid-cols-2'} sm:flex items-center gap-1 sm:gap-2 w-full py-1`}>
            {visibleTabs.map((tab) => {
              const Icon = tab.icon;
              ...
              return (
                <button
                  ...
                  className={`flex items-center justify-center sm:justify-start gap-1 sm:gap-2 px-1.5 sm:px-4 py-2 rounded-xl font-bold text-[11px] sm:text-xs whitespace-nowrap transition-all duration-150 cursor-pointer ${
                    isActive
                ...
                >
                  ...
                  {tab.badgeCount !== undefined && tab.badgeCount > 0 && (
                    <span className={`text-[9px] sm:text-[10px] font-bold px-1.5 py-0.5 rounded-full text-white shrink-0 ${tab.badgeColor || 'bg-indigo-600'}`}>
                      {tab.badgeCount}
                    </span>
                  )}
                </button>
